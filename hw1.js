...
/* document.getElementById('mobileUser').addEventListener('click', function() {
...
/* function onClickShop(event) {
  const wishlist = document.querySelector('#negozio');
  isVisibleShop = !isVisibleShop;
if (isVisibleShop) {
...
let isVisibleShop = false;

const shop = document.querySelector('#FBoxHeader');
shop.addEventListener('click', onClickShop); */
...
// Funzione per gestire la risposta JSON della ricerca
...
function search(event) {
  ...
  const album_input = document.querySelector('#ricerca');
...
// Credenziali OAuth --- NON SICURO!
// Assicurati di non includere mai credenziali sensibili in un codice pubblico
const client_id = 'b2842ee88cf24777a2d1e0821aa48b80';
const client_secret = 'e8533f0868364a7e960903a4d4b1257a';

let token;

// Richiediamo il token all'apertura della pagina
...
document.getElementById("Spotify").addEventListener("click", search);



//LOGIN E LOGOUT E NEGOZIO

document.addEventListener("DOMContentLoaded", function() {
  const userPanelToggle = document.getElementById("mobileUser");
  const logoutButton = document.getElementById("logoutBtn");
  const divAccesso = document.getElementById("login");
  const loginForm = document.querySelector('#LoginForm');
  const negozioDiv = document.getElementById("negozio");
  const shop = document.querySelector('#FBoxRitiraInNegozio');
  let isVisibleShop = false;

  if (userPanelToggle) {
    userPanelToggle.addEventListener("click", function(event) {
      const isLogged = userPanelToggle.getAttribute("data-logged-in") === "true";
  ...
  if (logoutButton) {
    logoutButton.addEventListener("click", async function(event) {
      ...
      if (response.ok) {
        ...
        logoutButton.style.display = "none";
        userPanelToggle.setAttribute("data-logged-in", "false");
        negozioDiv.classList.add('hidden');

        // Aggiunto codice per cancellare le credenziali dal form di login
        loginForm.reset();
      } else {
  ...
  loginForm.addEventListener('submit', async function(event) {
    ...
    if (result.success) {
      ...
      userPanelToggle.setAttribute("data-logged-in", "true");
      logoutButton.style.display = "block";
      divAccesso.style.display = "none";
    } else {
  ...
  function onClickShop(event) {
    ...
    if (!isLogged) {
      alert("Devi essere loggato per accedere al negozio.");
      return;
    }
    ...
    if (isVisibleShop) {
      negozioDiv.classList.remove('hidden');
      document.body.classList.add('no-scroll');
    } else {
      negozioDiv.classList.add('hidden');
      document.body.classList.remove('no-scroll');
...
const closeButton = document.querySelector('#chiudi-negozio');
closeButton.addEventListener('click', closeShop);
  
function closeShop() {
  const catalogo = document.querySelector('#negozio');
  isVisibleChiudi = false;
  catalogo.classList.add('hidden');
  document.body.classList.remove('no-scroll');
}

// AGGIUNTA E RIMOZIONE AI PREFERITI

document.addEventListener("DOMContentLoaded", function() {
  // Seleziona tutti i pulsanti con la classe btnHeart
  const buttons = document.querySelectorAll('.btnHeart');
  buttons.forEach(button => {
      button.addEventListener('click', function() {
          const productId = this.dataset.productId;  // Assumendo che l'ID del prodotto sia memorizzato come data attribute
          const isFavorite = this.classList.contains('favorited'); // Controlla se il pulsante è già stato contrassegnato come preferito
          if (isFavorite) {
              rimuoviDaPreferiti(productId, this); // Passa il riferimento al pulsante
          } else {
              aggiungiAiPreferiti(productId, this); // Passa il riferimento al pulsante
          }
      });
  });

  async function aggiungiAiPreferiti(productId, button) {
      try {
          const response = await fetch('aggiungiPreferiti.php', {
              method: 'POST',
              headers: {
                  'Content-Type': 'application/json'
              },
              body: JSON.stringify({ id_prodotto: productId }) // Cambiato da 'product_id' a 'id_prodotto'
          });
          
          const result = await response.json();
          if (result.success) {
              alert(result.message);
              // Aggiungi la classe 'favorited' al pulsante
  ...
  async function rimuoviDaPreferiti(productId, button) {
      try {
          const response = await fetch('rimuoviPreferiti.php', {
              method: 'POST',
              headers: {
                  'Content-Type': 'application/json'
              },
          ...
          if (result.success) {
              alert(result.message);
              // Rimuovi la classe 'favorited' dal pulsante
